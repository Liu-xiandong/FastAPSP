...
#include "fap/utils/util_centralized.h"
...
// -----------------step3----------------------
void diagBdyMessageBuild(int *C_diagBdy_num, int *C_diagBdy_offset,
    int K, int *C_BlockBdy_num, int &All_diagBdy_num) {
    int offset = 0;
    for (int i = 1; i <= K; i++) {
...
void subDiagMatBuild(float *subDiagMat, float *subMat,
    int bdy_num, int sub_vertexs) {
...
void add_edge(int a, int b, float c,
    vector<int> &h, vector<int> &e, vector<int> &ne,
    vector<float> &w, int &idx) {
    e.push_back(b);
    w.push_back(c);
    ne.push_back(h[a]);
    h[a] = idx++;
...
void BG_build_solver_path(
    int *adj_size, int *row_offset, int *col_val, float *weight,
    float *All_diagBdy, int bdy_vertexs,
    int *C_diagBdy_num, int *C_diagBdy_offset, int K,
...
void bdyMat2global(int *bdyMat_path,
    const int subBdy_vertexs, const int bdy_vertexs,
