...
__global__ void gemm_kernel(float *A, float *B, int *B_path, float *C, int *C_path, unsigned int m, unsigned int n, unsigned int k)
{
    //申请共享内存，存在于每个block中
...
extern "C" void gemm_AMD_path(float *mat1, float *mat2, int *mat2_path,
                              float *res, int *res_path, int tm, int tn, int tk)
{
    ...
    hipLaunchKernelGGL(gemm_kernel, dim3(GridDim_x, GridDim_y), dim3(blockDim, blockDim), 0, 0, d_a, d_b, d_b_path, d_c, d_c_path, m, n, k);
                                   ...
                                   int start, int sub_vertexs, int vertexs, int *st2ed)
{
    ...
    for (int i = 0; i < part_num; i++)
    {
        ...
        hipLaunchKernelGGL(gemm_kernel, dim3(GridDim_x, GridDim_y), dim3(blockDim, blockDim), 0, 0, d_a, d_b, d_b_path, d_c, d_c_path, now_size, n, k);
...
extern "C" void gemm_AMD_GPU(float *mat1, float *mat2, float *res,
							 int m, int n, int k)
{
	float *d_a;
	float *d_b;
	...
	hipLaunchKernelGGL(gemm_kernel, dim3(GridDim_x, GridDim_y), dim3(blockDim, blockDim), 0, 0, d_a, d_b, d_c, m, n, k);
